...
##### Q1: How do you decide whether a problem should be modeled as regression, classification, or ranking?
...
##### Q2: What factors guide your choice between a simple model (e.g. logistic regression) and a complex model (e.g. XGBoost or neural nets)?
...
##### Q3: Explain the bias-variance tradeoff. How does model complexity affect it?

The **expected loss** (square error for example) of a model, which we try to minimize, can be decomposed into three components:
...
1. **Regularization** (L1/L2) to reduce variance in flexible models
2. **Ensemble methods**: 
   - bagging to reduce variance, 
   - boosting to reduce bias
3. **Cross-validation** to detect overfitting or underfitting
...
##### Q4: How do you decide what features to create or include in a machine learning model?
...
- **Aggregations across groups**: mean, sum, counts (e.g., customer-level stats)
...
##### Q5: How does logistic regression work? What is its loss function and why?
...
##### Q6: How do you evaluate a regression/classification model? 
...
- **Recall (Sensitivity)**: Of all actual positives, how many did the model identify? *Useful when missing positives is costly*.
- **F1 Score**: Harmonic mean of precision and recall; balances the two metrics, more leaned towards the smaller value
- **Accuracy**: Overall *proportion of correctly classified instances*.
- **AUC-ROC**: Measures ability of the model to distinguish classes *across thresholds*. 
 
###### What is ROC curve? 
The ROC curve (Receiver Operating Characteristic curve) **plots the True Positive Rate (TPR) against the False Positive Rate (FPR) at various classification thresholds**. By changing the threshold that decides whether a prediction is positive or negative, you get different TPR and FPR values, which form the ROC curve.
...
Since ROC is inherently binary, for multi-class problems, common approaches are:
...
##### Q7: How do you handle high cardinality categorical features in a machine learning model?
...
##### Q8: How do you detect and handle outliers in a dataset? What methods do you prefer and when?
...
##### Q9: What is the difference between L1 and L2 regularization? When would you choose one over the other?
...
##### Q10: What’s data leakage? Can you give an example from your experience or projects?
...
##### Q11: How do you handle missing data in a dataset? What are the pros and cons of different methods?
...
##### Q12: When Would You Use Logistic Regression vs. Decision Trees?
...
##### Q14: Explain how k-NN works and its pros/cons.?
...
##### Q15: What is the difference between decision trees, random forests, and gradient boosting?
...
##### Q16: How does LightGBM work, and how is it different from XGBoost?
...
##### Q17: What are kernel tricks in SVMs?
...
##### Q18: How would you deal with categorical variables in a dataset with many low-frequency categories?
...
- Use techniques like **entity embeddings (for deep models)** or **hashing tricks** for scalable solutions.
...
##### Q19: What metrics would you use for an imbalanced binary classification problem? How do you evaluate multi-class?
...
- **Macro F1**: Unweighted **average  of individual class F1** scores (treats all classes equally)
...
##### Q20: Why is cross-validation important? What are the pros and cons of K-fold vs stratified K-fold?
...
##### Q21: Tell me about a project where you had to tune hyperparameters. What was your approach?
...
##### Q22: How do you choose and tune hyperparameters for a model?
...
##### Q23: What are the most common optimization algorithms used in deep learning? How do Adam and SGD differ?
Common optimizers:
...
In production, we often start with Adam for quick convergence, then switch to SGD for fine-tuning.

##### Q24: Explain batch normalization and why it helps.
...
###### Reasoning:
BN normalizes the pre-activation values (the weighted inputs). This ensures that the activation function (like ReLU, GELU, etc.) receives inputs with a stable distribution — avoiding saturation regions and helping gradients flow smoothly. If BN were applied after activation (especially ReLU), half the values would be zero, **distorting the mean/variance estimates** and reducing its stabilizing effect.
...
##### Q25: How do you evaluate regression models?
...
- Good Fit (Ideal Scenario):
  - A r**andom scatter of points around the x-axis** (residual = 0). 
  - A constant width band of points (constant variance). 
...
##### Q26: How do you deal with multi-collinearity in regression?
...
- Also check **condition number** or e**xamine eigenvalues of the design matrix** $X^TX$
2. Solutions
...
- High but not perfect collinearity:
  - If correlation > 0.95–0.97, consider:
    ...
    - Combining them (e.g., PCA, feature engineering) to **decorrelate** them
...
##### Q27: Suppose your model performs well on training data but poorly on test data. What would you do?
...
##### Q28: How would you explain a model’s predictions to a non-technical stakeholder?
...
##### Q29: What are different ways to determine feature importance in a model?
...
##### Q30: How do you choose the right evaluation metric for a given task?
...
Metrics depend on whether:
   ...
   - Relative error matters:
       - Use MAPE (caution: only if no zeros in true values)

2. **Classification Tasks**
    Depends on class balance and decision thresholds:
    - **Balanced data**: Accuracy, Cross-Entropy Loss
    - **Imbalanced data**: Use Precision, Recall, F1-score, AUC ROC, or PR-AUC
...
- For clustering:
    - Use Silhouette Score, Adjusted Rank Index, or Davies–Bouldin Index
...
##### Q31: How do you validate your model to ensure it generalizes well?
...
##### Q32: What are some techniques to deal with imbalanced datasets?
...
2. **Model-level techniques**:
    ...
    - Using algorithms that support imbalance handling natively. XGBoost or LightGBM allow setting `scale_pos_weight`
...
##### Q33: You’re working with messy real-world data (missing values, noise, imbalance, unclear signal). How do you approach building a model in such a scenario?
...
##### Q34: What are common methods to prevent overfitting in deep learning?
...
- **Dropout**: Randomly setting a fraction of neurons output to zero during each training forward pass, preventing co-adaptation and encouraging generalization.
...
##### Q35: When would you use log loss vs. mean squared error (MSE)?
...
Softmax outputs in multi-class classification, binary classifiers with sigmoid outputs are common examples for log loss application
...
##### Q36: How do you monitor model performance in production?
...
##### Q37: How do you handle data drift or concept drift after deployment?
...
##### Q38: What’s your approach for serving models in real-time vs batch inference pipelines?
...
##### Q39: How do you evaluate a model in production when ground truth is not avaialble?
...
##### Q40: How would you design a pipeline to train and serve a machine learning model that needs frequent retraining due to rapidly changing data (e.g., fraud detection or real-time recommendation)?

In use cases like fraud detection or recommendation, where data distributions change rapidly, I’d design an **automated ML pipeline** to support frequent retraining and robust deployment. 
...
##### Q41: Explain the Transformer architecture — why was it a breakthrough compared to RNNs and CNNs?
...
##### Q42: What are self-attention and multi-head attention? Why multiple heads?

**Self-attention computes how much each token should focus on every other token in the same sequence**.

For each token we create **Query**, **Key**, and **Value** vectors and compute attention weights via **scaled dot products** which calculates the **similarity between tokens**.

<p align="center">
<img src="../assets/interview-projects/self-attention.png" alt="drawing" width="600" height="250" style="center" />
</p>
...
##### Q43: How does positional encoding work, and why is it necessary?
...
**Positional encodings inject sequence order by adding *deterministic fixed-sized* or *learned* position vectors to token embeddings**. The original paper used **sinusoidal encodings** so the model can generalize to longer sequences — they allow the model to *infer relative positions* using smooth, periodic patterns.


##### Q44: What is causal masking and why do we use it in decoder-only models like GPT?
...
##### Q45: Compare encoder-decoder (T5/BART) vs decoder-only (GPT) vs encoder-only (BERT) architectures.
...
##### Q46: How does layer normalization differ from batch normalization, and why is it preferred in transformers?
...
##### Q47: What is residual connection, and why is it crucial in deep networks like transformers?
...
- In Transformers, e**ach attention or feed-forward block is wrapped with a residual + LayerNorm**, enabling networks with dozens or hundreds of layers to converge reliably.

##### Q48: What are attention masks (padding, causal) — when and why are they used?
...
##### Q49: What’s the difference between pretraining, supervised fine-tuning, and RLHF?
...
##### Q50: How is data prepared for LLM pretraining?
...
##### Q51: What’s the difference between masked language modeling (MLM) and causal language modeling (CLM)?
...
##### Q52: What are the key steps in fine-tuning a pretrained transformer on a downstream NLP task?
...
##### Q53: What is instruction tuning? How is it different from standard fine-tuning?
...
##### Q54: What are the challenges in fine-tuning large models?
...
##### Q55: What is LoRA (Low-Rank Adaptation) and why is it efficient for fine-tuning?

What is LoRA (Low-Rank Adaptation)? Why is it used?

###### Goal 
Fine-tune large models efficiently by **training a small set of additional parameters**.

###### How it works:
  - Instead of updating full weight matrix $W$, LoRA adds two small matrices  (**low rank**) $A$, $B$ such that $A \times B$ has the **same size** as $W$. 
   - **Only  $A$, $B$ are trained**;  $W$ **stays frozen**.

###### Benefits:
- **Reduces memory & compute** dramatically.
- **Easy to merge/unmerge LoRA layers for inference**.
- Used in RLHF, domain adaptation, chat fine-tuning without retraining base LLM.

##### Q56: What are PEFT methods, and how do they differ from full fine-tuning?
...
##### Q57: What is quantization? How does it help in deploying LLMs?
...
###### How it helps:
- **Model fits in smaller GPU/CPU RAM**.
...
##### Q58: What’s the difference between quantization, pruning, and distillation?
...
##### Q59: How would you deploy an LLM for real-time inference?
- **Model Optimization**:
...
##### Q60: How do you evaluate an LLM during or after fine-tuning?
...
##### Q61: “What is feedback loop, retention, and qualitative satisfaction?”
...
##### Q62: Explain prompt tuning, prefix tuning
...
##### Q63:  What are Catastrophic Forgetting and Continual Learning, and how do we mitigate them during fine-tuning?
...
##### Q64: Can you explain how Mixture of Experts differs from LoRA stacking? 
...
MoE architectures replace some dense layers (often feed-forward) with sparse expert layers — multiple expert subnetworks, each specialized for different inputs. A gating network decides which experts to activate for a given token.
...
##### Q65: What is KV caching and why is it critical for LLM inference?
...
##### Q66: What is FlashAttention and how does it improve Transformer efficiency?
...
<!-- ##### Q67: What are quantization techniques and how do they impact inference?
...
##### Q67: How do frameworks like vLLM or TensorRT-LLM accelerate inference?
...
##### Q69: What’s the trade-off between low latency and high throughput in model serving?
...
<!-- ##### Q70: How do large models ensure efficient inference at scale (serving optimization)?
Key techniques:
...
##### Q70: What are common issues in production LLM usage and mitigation?
...
##### Q71: What is an activation weight in transformers? and what is activation checkpointing and why is it used?

**Activation weights** usually refer to the intermediate activations — **the outputs of each layer** (e.g., hidden states after attention or MLP blocks) that d**epend on both model weights and input tokens**.
...
##### Q72: How do model parallelism, data parallelism, and pipeline parallelism differ?
...
##### Q73: Explain ZeRO optimization from DeepSpeed — what problems does it solve?
...
##### Q74: How do mixed-precision training (FP16/BF16) and gradient scaling improve performance?
...
#### Evaluation, Alignment, and Deployment


##### Q76: What is RLHF? Explain its 3 stages (SFT, Reward Model, PPO).
...
##### Q77: What are the limitations of PPO in RLHF training?
...
##### Q78: How do you deploy large models efficiently (consider latency, batching, caching, quantization)?
...
##### Q79: Explain shadow evaluation and A/B testing for LLMs in production.
...
##### Q80: What is QLoRA, and why is it important in fine-tuning large models?
...
##### Q81: What are common metrics used to evaluate hallucination in LLMs?
...
##### Q82: How do you evaluate a generative model like a language model or image generator?
...
------------
####  Advanced / Real-World & Conceptual


##### Q83: What is Retrieval-Augmented Generation (RAG), and when is it useful?

**Retrieval-Augmented Generation (RAG)** is a method that enhances LLMs by giving them **access to external**, **factual information at inference time** — without retraining or fine-tuning the model. RAG combines a **retriever** (fetches relevant documents) with a **generator** LLM to produce output conditioned on retrieved context).
...
- **Data is private**, proprietary, or customer-specific
- **Fine-tuning a large model is too expensive or slow**

Use cases: chatbots, customer support, document QA, and enterprise search
...
| **Embedding**   | Sentence-Transformers, OpenAI embeddings, Contriever |
...
##### Q84: How do you evaluate the quality of a RAG system? Whether its hallucinating or not?
...
If ground truth answers exist, use:
   ...
   - Use LLM-as-a-judge evaluation:
      ...
      - Ask a strong model to rate responses for **relevance**, **correctness**, **helpfulness**, **hallucination**. 
...
##### Q85: How do you detect or mitigate hallucinations in RAG outputs?
Hallucination = confident but false information.

Mitigation strategies:
1. **Retrieval**:
     - Retrieval grounding: Ensure the **LLM explicitly references retrieved context**; use prompt templates like:
       - “Answer only using the context provided below. If unsure, say ‘Not enough information.”
   - Retriever should use high quality embeddings and metadata filtering

2. **Post-generation verification**:
- **LLM-as-judge**: Use a checker model for factuality, correctness etc.
- Use n**amed entity verification with APIs or databases**.

1. **Reward models or RLHF**:
Train a model to penalize ungrounded or false statements.

1. **Structured prompting**:
Use **chain-of-thought** or **citation-based prompting** forces model to show reasoning.


<!-- ##### Q83: How does fine-tuning compare to RAG for domain adaptation?
...
##### Q86: How do you optimize the retrieval step in RAG for relevance and efficiency?
...
##### Q87: What role does the embedding model play in RAG pipelines? How do you choose or benchmark an embedding model?
...
##### Q87: How do you evaluate the factuality or grounding quality of a RAG or LLM system?
...
<!-- 
##### Q86: What are trade-offs between retrieval window size and context quality in RAG systems?
...
##### Q88: How would you monitor LLM drift or toxicity post-deployment?
...
##### Q89: What to monitor in RAG systems in Production what are Common Failures?
...
<!-- ##### Q88: How do you combine RAG with fine-tuning effectively?
...
##### Q90: Why Reranking Is a Good Idea (Cross-Encoder vs FAISS) in RAG systems
...
- However, similarity ≠ semantic relevance perfectly — it only measures geometric closeness of embeddings, not deep reasoning between query and document. Example:
...
##### Q90: What is BERTScore, FActScore, TRUE?
...
###### Idea 
Instead of comparing whole-sentence embeddings, it compares each token of the generated answer to the most similar token in the reference answer — using contextual embeddings from a pretrained model like BERT, RoBERTa, or DeBERTa.
...
$$
\text{FActScore}= \frac{\# of supported claims}{Total \# of claims}
$$​	
...
TRUE is a benchmark and composite metric for truthfulness and factual consistency across LLM tasks. It’s not one model, but rather a framework that aggregates results from multiple NLI, QA, and summarization factuality metrics. TRUE gives you access to a set of tested factuality scorers. 
...
Option2:
"Based on the role, responsibilities, and the market for data analyst positions involving AI/ML in Toronto, I would expect a salary in the range of 80K to 100K, depending on the overall compensation package and benefits. Of course, I’m flexible and open to discussion."
...
##### Q: Compare HuggingFace TGI with vLLM for inference workloads?
...
- Infrastructure as Code (IaC)
   All cloud resources — compute, networking, storage, IAM — are provisioned using tools like Terraform or AWS CDK, ensuring the infra is always replicable.
...
- Pipeline Automation
Automate data preprocessing, feature engineering, training, and evaluation with reproducible pipelines using orchestrators  like Airflow, Kubeflow, or MLflow or CI/CD tools (e.g., GitHub Actions, CodePipeline) to automate data ingestion, training, evaluation, and deployment. No manual steps.
